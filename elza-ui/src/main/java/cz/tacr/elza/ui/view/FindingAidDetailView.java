...
import cz.tacr.elza.ui.window.DirectionLevel;
...
public class FindingAidDetailView extends ElzaView implements PosAction {
    ...
    public void enter(final ViewChangeListener.ViewChangeEvent event) {
        ...
        HierarchicalCollapsibleBeanItemContainer container = new HierarchicalCollapsibleBeanItemContainer(null,
                rootNode.getNode().getNodeId());


        List<RulDescItemType> sloupce = new LinkedList<>();
        if (version != null) {
            FaViewDescItemTypes faViewDescItemTypes = ruleSetManager
                    .getFaViewDescItemTypes(version.getFindingAidVersionId());
        ...
        table.addExpandListener(new Tree.ExpandListener() {
            ...
            public void nodeExpand(final Tree.ExpandEvent expandEvent) {
                ...
                List<ArrLevel> faLevels = arrangementManager
                        .findSubLevels(itemId.getNode().getNodeId(), version.getFindingAidVersionId());
                ...
                List<ArrLevelExt> faLevelsExt = arrangementManager
                        .findSubLevels(itemId.getNode().getNodeId(), version.getFindingAidVersionId(),
                                ArrangementManager.FORMAT_ATTRIBUTE_SHORT, null);
        ...
        table.addGeneratedColumn("Akce", new Table.ColumnGenerator() {
            ...
            public Object generateCell(final Table source, final Object itemId, final Object columnId) {
                ...
                MenuBar.MenuItem child = new AxAction().caption("Přidat záznam před").icon(FontAwesome.PLUS).run(() -> {
                            ...
                            AddLevelScenariosWindow addLevelScenariosWindow = new AddLevelScenariosWindow(arrangementManager);

                            addLevelScenariosWindow.show("Přidat záznam před", (ArrLevel) itemId,
                                    version, DirectionLevel.BEFORE, scenarioOfNewLevel -> {

                                        discardNodeCut();

                                        ArrLevel node = (ArrLevel) itemId;
                                        ArrLevelWithExtraNode faLevelWithExtraNode = new ArrLevelWithExtraNode();
                                        faLevelWithExtraNode.setDescItems(scenarioOfNewLevel.getDescItems());
                                        ...
                                        try {
                                            RelatedNodeDirectionWithLevelPack relatedNodeDirectionWithLevelPack
                                                    = arrangementManager
                                                    .addLevelBefore(faLevelWithExtraNode);
                                            ArrLevelWithExtraNode faLevelWithExtraNodeRet
                                                    = relatedNodeDirectionWithLevelPack.getArrLevelPack();

                                            showRelatedNodeDirection(
                                                    relatedNodeDirectionWithLevelPack.getRelatedNodeDirections());
                ...
                child = new AxAction().caption("Přidat záznam za").icon(FontAwesome.PLUS).run(new Runnable() {
                    ...
                    public void run() {
                        ...
                        addLevelScenariosWindow.show("Přidat záznam za", (ArrLevel) itemId,
                                version, DirectionLevel.AFTER, scenarioOfNewLevel -> {
                ...
                child = new AxAction().caption("Přidat podřízený záznam").icon(FontAwesome.PLUS).run(() -> {
                    ...
                    addLevelScenariosWindow.show("Přidat podřízený záznam", (ArrLevel) itemId,
                            version, DirectionLevel.CHILD, scenarioOfNewLevel -> {
    ...
    private void refreshParentReferences(ArrLevel node,
                                         ArrLevelWithExtraNode faLevelWithExtraNodeRet,
    ...
    private void refreshTree(final HierarchicalCollapsibleBeanItemContainer container, final ArrLevel rootLevel) {
        ...
        List<ArrLevelExt> faLevelsExt = arrangementManager
                .findSubLevels(version.getRootLevel().getNode().getNodeId(), version.getFindingAidVersionId(),
                        ArrangementManager.FORMAT_ATTRIBUTE_SHORT, null);
        for (ArrLevelExt faLevel : faLevelsExt) {
            addAttributeToCache(faLevel);
        }
     ...
     */
    private ArrLevel getItemIdAfterChilds(final ArrLevel itemId,
    ...
    private ArrLevel addItemAfterToContainer(final ArrLevel level,
        ...
        if (!container.isCollapsed(level)) {
            List<ArrLevelExt> faLevelsExt = arrangementManager
                    .findSubLevels(level.getNode().getNodeId(), version.getFindingAidVersionId(),
                            ArrangementManager.FORMAT_ATTRIBUTE_SHORT, null);
            for (ArrLevelExt faLevel : faLevelsExt) {
                addAttributeToCache(faLevel);
            }
        ...
        if (historiOnly) {
            ...
            actions(
                    new AxAction().caption("Přidat záznam").icon(FontAwesome.PLUS).run(() -> {
                        ...
                        addLevelScenariosWindow.show("Přidat záznam", version.getRootLevel(),
                                version, DirectionLevel.CHILD,
                                scenarioOfNewLevel -> {
                                    ArrLevelWithExtraNode faLevelWithExtraNode = new ArrLevelWithExtraNode();
                                    faLevelWithExtraNode.setDescItems(scenarioOfNewLevel.getDescItems());
