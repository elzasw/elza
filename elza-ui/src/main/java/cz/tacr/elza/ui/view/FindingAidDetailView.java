...
//                return property == null || property.getValue() == null ? null : property.getValue().toString();
        ...
        table.addGeneratedColumn("Akce", new Table.ColumnGenerator() {
            ...
            public Object generateCell(final Table source, final Object itemId, final Object columnId) {
                ...
                child = new AxAction().caption("Smazat").icon(FontAwesome.TRASH_O).run(new Runnable() {
                    ...
                    public void run() {
                        ...
                        for (ArrFaLevel faLevel : childs) {
                            ...
                            if (faLevel.getPosition() >= node.getPosition()) {
                                faLevel.setVersion(faLevel.getVersion() + 1);
                            }
                        }
                ...
                child = new AxAction().caption("Vložit před").icon(FontAwesome.PASTE).run(() -> {
                    try {
                        if (checkPaste()) {
                            ...
                            faLevelWithExtraNode.setExtraNode(((ArrFaLevel) itemId).getParentNode());
                            faLevelWithExtraNode.setRootNode(version.getRootFaLevel().getNode());
                            ArrFaLevelWithExtraNode faLevelWithExtraNodeRet = arrangementManager.moveLevelBefore(faLevelWithExtraNode);
                            ArrFaLevel level = faLevelWithExtraNodeRet.getFaLevel();
                ...
                child = new AxAction().caption("Vložit za").icon(FontAwesome.PASTE).run(() -> {
                    try {
                        if (checkPaste()) {
                            ...
                            faLevelWithExtraNode.setExtraNode(((ArrFaLevel) itemId).getParentNode());
                            faLevelWithExtraNode.setRootNode(version.getRootFaLevel().getNode());
                            ArrFaLevelWithExtraNode faLevelWithExtraNodeRet = arrangementManager.moveLevelAfter(faLevelWithExtraNode);
                            ArrFaLevel level = faLevelWithExtraNodeRet.getFaLevel();
                ...
                child = new AxAction().caption("Vložit jako podřízený").icon(FontAwesome.PASTE).run(() -> {
                    try {
                        if (checkPaste()) {
                            ...
                            ArrFaLevel level = faLevelWithExtraNodeRet.getFaLevel();
                            //((ArrFaLevel) itemId).setParentNode(faLevelWithExtraNodeRet.getExtraNode());
                ...
                child = new AxAction().caption("Zobrazit historii").icon(FontAwesome.CALENDAR).run(() -> {
                    showVersionHistory((ArrFaLevel) itemId);
                }).menuItem(parent);
                return menuBar;
            }
        });
    ...
    private LevelHistoryWindow showVersionHistory(final ArrFaLevel level) {
        LevelHistoryWindow window = new LevelHistoryWindow(arrangementManager);
        window.show(level, findingAidId);
        return window;
    }
