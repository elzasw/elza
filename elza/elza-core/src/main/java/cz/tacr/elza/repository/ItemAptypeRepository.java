...
public interface ItemAptypeRepository extends ElzaJpaRepository<RulItemAptype, Integer> {
    ...
    @Query("SELECT ap FROM rul_item_aptype ap WHERE ap.itemSpec in (?1)")
    List<RulItemAptype> findByItemSpecs(Collection<RulItemSpec> itemSpecs);
     ...
     * @return vrací registry typu
     */
    @Query("SELECT ap FROM rul_item_aptype ap WHERE ap.itemType in (?1)")
    List<RulItemAptype> findByItemTypes(Collection<RulItemType> itemTypes);
