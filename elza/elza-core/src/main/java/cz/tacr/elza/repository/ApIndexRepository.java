...
public interface ApIndexRepository extends JpaRepository<ApIndex, Integer> {
    ...
    @Query("SELECT i FROM ap_index i JOIN i.part p WHERE p.partType = :partType AND i.indexType = :indexType")
    ApIndex findByPartTypeAndIndexType(@Param("partType") RulPartType partType, @Param("indexType") String indexType);
    ...
    @Query("SELECT i FROM ap_index i JOIN FETCH i.part p JOIN p.accessPoint ap WHERE ap IN :accessPoints AND p.partType = :partType AND i.indexType = :indexType")
