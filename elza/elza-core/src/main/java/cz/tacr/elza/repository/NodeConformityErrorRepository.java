...
public interface NodeConformityErrorRepository extends JpaRepository<ArrNodeConformityError, Integer> {
    ...
    @Query("SELECT c FROM arr_node_conformity_error c JOIN FETCH c.nodeConformity n WHERE c.descItemId in (?1)")
    List<ArrNodeConformityError> findByItemIds(Collection<Integer> conformityInfoIds);
