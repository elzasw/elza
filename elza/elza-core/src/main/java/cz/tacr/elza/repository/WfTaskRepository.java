...
public interface WfTaskRepository extends ElzaJpaRepository<WfTask, Integer> {
	...
	@Query("select t from wf_task t join fetch t.taskType where t.assigneeId = :assigneeId and t.status = 'NEW'")
	List<WfTask> findNewByAssigneeId(@Param("assigneeId") Integer assigneeId);
