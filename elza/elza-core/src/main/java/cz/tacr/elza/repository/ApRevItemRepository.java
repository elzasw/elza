...
public interface ApRevItemRepository extends JpaRepository<ApRevItem, Integer> {
    ...
    @Query("SELECT i FROM ApRevItem i LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType WHERE i.part IN :parts AND i.deleteChange IS NULL")
    List<ApRevItem> findByParts(@Param("parts") Collection<ApRevPart> parts);

    @Query("SELECT i FROM ApRevItem i LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType WHERE i.part = :part AND i.deleteChange IS NULL")
