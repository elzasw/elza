...
public interface OutputTemplateRepository extends ElzaJpaRepository<ArrOutputTemplate, Integer> {
    ...
    @Query("SELECT ot FROM arr_output_template ot JOIN FETCH ot.template t WHERE ot.output.outputId=?1")
    List<ArrOutputTemplate> findAllByOutputIdFetchTemplate(int outputId);
