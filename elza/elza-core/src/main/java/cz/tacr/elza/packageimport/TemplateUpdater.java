...
public class TemplateUpdater {
    ...
    public void run(RuleUpdateContext ruc) {
        ...
        for (Entry<RulTemplate, RulTemplate> itemMapping : otherTemplMapping.entrySet()) {
            ...
            outputTemplateRepository.updateTemplateByTemplate(itemMapping.getKey(), itemMapping.getValue());
        }
        ...
        if (!rulTemplateToDelete.isEmpty()) {
            ...
            List<ArrOutputTemplate> byTemplate = outputTemplateRepository
            ...
            if (!byTemplate.isEmpty()) {
                ...
                byTemplate.forEach((ot) -> {
                    ArrFund fund = ot.getOutput().getFund();
                    sb.append("\noutputId: ").append(ot.getOutputId())
                            .append(", outputName: ").append(ot.getOutput().getName())
