...
public interface WfTaskApStateRepository extends ElzaJpaRepository<WfTaskApState, Integer> {
	...
	/**
	 * Returns distinct AP state ids that have an open task assigned to the given user.
	 * Used by registry search to pre-resolve the assignedTo filter into an IN list.
	 * The pageable is expected to carry a LIMIT so the caller can detect overflow.
	 */
	@Query("select distinct ts.stateId from wf_task_ap_state ts join ts.task t "
			+ "where t.assigneeId = :assigneeId and t.timeClosed is null")
	List<Integer> findStateIdsByAssignee(@Param("assigneeId") Integer assigneeId, Pageable pageable);
}
