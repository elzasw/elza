...
public interface ApItemRepository extends JpaRepository<ApItem, Integer> {
    ...
    @Query("SELECT i FROM ApItem i JOIN FETCH i.part p JOIN FETCH p.accessPoint JOIN FETCH i.itemType it JOIN FETCH it.dataType JOIN arr_data_record_ref d ON i.data = d WHERE d.binding = :binding AND i.deleteChange IS NULL")
