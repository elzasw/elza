...
import cz.tacr.cam.v2.schema.cam.CreateEntityXml;
import cz.tacr.cam.v2.schema.cam.EntityRecordStateXml;
import cz.tacr.cam.v2.schema.cam.EntityRefXml;
...
public class CreateEntityBuilder extends BatchUpdateBuilder {
    ...
    public boolean build(List<ApPart> srcPartList, Map<Integer, List<ApItem>> itemMap) {
        ...
        if (xmlParts == null || xmlParts.getPart().size() == 0) {
        	return false;
        }

        createEntity = new CreateEntityXml();
        createEntity.setLocalId("LID" + apState.getAccessPointId());
        createEntity.setEntityType(new CodeXml(apState.getApType().getCode()));
        createEntity.setEntityUuid(new UuidXml(accessPoint.getUuid()));
        createEntity.setParts(xmlParts);
