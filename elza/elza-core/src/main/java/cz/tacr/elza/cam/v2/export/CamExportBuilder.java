...
import cz.tacr.elza.cam.v2.EntityXmlBuilder;
...
public class CamExportBuilder implements ExportBuilder {
    ...
    public void addAccessPoint(ApInfo apInfo) {
        ...
        this.entities.getEntity().add(ent);
    }
    ...
    public void build(OutputStream os) throws XMLStreamException {
        JAXBElement<EntitiesXml> jaxbEnts = CamUtils.getObjectFactory().createEntities(entities);

        try {
            Marshaller marshaller = jaxbContext.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.setSchema(schemaManager.getSchema(SchemaManager.CAM_SCHEMA_2025));
    ...
    public void buildEntity(Writer writer) throws XMLStreamException {
        Validate.isTrue(this.entities.getEntity().size() == 1);
        EntityXml entXml = this.entities.getEntity().get(0);

        JAXBElement<EntityXml> jaxbEnt = CamUtils.getObjectFactory().createEntity(entXml);
    ...
    public void buildEntity(Document xmlDoc) throws XMLStreamException {
        Validate.isTrue(entities.getEntity().size() == 1);
        EntityXml entXml = entities.getEntity().get(0);
