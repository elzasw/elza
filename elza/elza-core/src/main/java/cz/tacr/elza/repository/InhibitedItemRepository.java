...
public interface InhibitedItemRepository extends JpaRepository<ArrInhibitedItem, Integer> {
    ...
    List<ArrInhibitedItem> findByDescItemObjectIdAndDeleteChangeIsNull(Integer descItemObjectId);
    ...
    @Query("SELECT i FROM arr_inhibited_item i WHERE i.deleteChange IS NULL AND i.node IN (SELECT n FROM arr_node n WHERE n.fund = ?1)")
    List<ArrInhibitedItem> findOpenByNodeFund(ArrFund fund);
