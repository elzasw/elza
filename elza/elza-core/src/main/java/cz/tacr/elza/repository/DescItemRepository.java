...
public interface DescItemRepository extends ElzaJpaRepository<ArrDescItem, Integer>, DescItemRepositoryCustom {
	...
	@Query("SELECT i FROM arr_desc_item i WHERE i.node in (?1) AND i.deleteChange IS NULL AND i.itemType = ?2")
    List<ArrDescItem> findOpenByNodesAndType(Collection<ArrNode> nodes, RulItemType type); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType JOIN FETCH i.node 

	@Query("SELECT i FROM arr_desc_item i WHERE i.node in (?1) AND i.deleteChange IS NULL AND i.itemType = ?2 AND i.itemSpec IN (?3)")
    List<ArrDescItem> findOpenByNodesAndTypeAndSpec(Collection<ArrNode> nodes, RulItemType type, Collection<RulItemSpec> specs); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType JOIN FETCH i.node 

	@Query("SELECT i FROM arr_desc_item i JOIN FETCH i.node n WHERE n.fund = :fund AND i.deleteChange IS NULL AND i.itemType = :type")
    List<ArrDescItem> findOpenByFundAndType(@Param("fund") ArrFund fund,
                                            @Param("type") RulItemType type); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType 

	@Query("SELECT i FROM arr_desc_item i JOIN FETCH i.node n WHERE n.fund = :fund AND i.deleteChange IS NULL AND i.itemType = :type AND i.itemSpec IN :specs")
	...
	@Query("SELECT i FROM arr_desc_item i WHERE i.nodeId IN (?1) AND i.itemTypeId IN (?2) AND i.deleteChange IS NULL")
...
//    @Query("SELECT i FROM arr_desc_item i JOIN i.itemType t JOIN i.itemSpec s WHERE i.node = ?1 AND i.deleteChange IS NULL AND i.itemTypeId = ?2 AND s.itemSpecId = ?3")
//    List<ArrDescItem> findByNodeAndDeleteChangeIsNullAndItemTypeIdAndSpecItemTypeId(ArrNode node, Integer itemTypeId, Integer itemSpecId);
	 ...
	 */
	@Query("SELECT i FROM arr_desc_item i WHERE i.node = ?1 AND i.deleteChange IS NULL AND i.itemTypeId = ?2")
    List<ArrDescItem> findByNodeAndDeleteChangeIsNullAndItemTypeId(ArrNode node, Integer descItemTypeId); // exclude: JOIN FETCH i.itemType t JOIN FETCH t.dataType 
	...
	List<ArrDescItem> findByNodeAndChange(ArrNode node, ArrChange change); // exclude: JOIN FETCH i.itemType t LEFT JOIN FETCH t.dataType 

    @Query("SELECT i FROM arr_desc_item i JOIN i.itemType t WHERE i.node = ?1 AND i.itemTypeId = ?2 AND i.createChange < ?3 AND (i.deleteChange > ?3 OR i.deleteChange IS NULL)")
	 ...
	 */
    @Query("SELECT i FROM arr_desc_item i WHERE i.node = ?1 AND i.deleteChange IS NULL AND i.itemType IN (?2) ORDER BY i.position")
    List<ArrDescItem> findOpenByNodeAndTypes(ArrNode node, Set<RulItemType> descItemTypes); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType 
     ...
     */
	@Query("SELECT i FROM arr_desc_item i WHERE i.deleteChange IS NULL AND i.descItemObjectId = ?1")
    ArrDescItem findOpenDescItem(Integer descItemObjectId); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType 
    ...
    List<ArrDescItem> findOpenDescItemsAfterPosition(RulItemType itemType, ArrNode node, Integer position); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType  
    ...
    List<ArrDescItem> findOpenDescItemsByItemType(RulItemType itemType, ArrNode node); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType  
     ...
     */
    @Query(value = "SELECT max(i.position) FROM arr_desc_item i JOIN i.itemType t WHERE i.node = ?1 AND i.itemTypeId = ?2 AND i.deleteChange IS NULL")
     ...
     */
    @Query("SELECT i FROM arr_desc_item i JOIN i.itemType t WHERE i.position >= ?1 AND i.node = ?2 AND i.deleteChange IS NULL AND i.itemTypeId = ?3")
     ...
     * @return nalezené Atributy archivního popisu.
     */
    @Query("SELECT i FROM arr_desc_item i JOIN i.itemType t WHERE i.position > ?1 AND i.position <= ?2 AND i.node = ?3 AND i.deleteChange IS NULL AND i.itemTypeId = ?4")
     ...
     */
    @Query("SELECT i FROM arr_desc_item i WHERE i.deleteChange IS NULL AND i.itemType = :itemType AND i.node = :node AND i.position >= :positionFrom AND i.position <= :positionTo")
    List<ArrDescItem> findOpenDescItemsBetweenPositions(@Param("itemType") RulItemType itemType,
                                                        ...
                                                        @Param("positionTo") int positionTo); // exclude: LEFT JOIN FETCH i.itemType it LEFT JOIN FETCH it.dataType 
