...
public interface DescItemRepository extends ElzaJpaRepository<ArrDescItem, Integer>, DescItemRepositoryCustom {
     ...
     * @return
     */
    @Query("SELECT i FROM arr_desc_item i JOIN FETCH i.itemType it JOIN FETCH it.dataType where i in (SELECT i FROM arr_desc_item i JOIN ArrDataUriRef d on i.data=d WHERE d.arrNode = :node AND i.deleteChange IS NULL)")
