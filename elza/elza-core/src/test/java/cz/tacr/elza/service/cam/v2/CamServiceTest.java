...
import cz.tacr.elza.cam.v2.CamService;
...
public class CamServiceTest extends AbstractControllerTest {
    ...
    static {
    	...
    	IMPORT_ENTITIES.getEntity().add(ent1);
    	// ent2
    	...
    	refPart.getItems().getItems().add(refItem);
    	ent2.getParts().getPart().add(refPart);

    	IMPORT_ENTITIES.getEntity().add(ent2);
    }

	private static EntityXml createEntity(Long extId, String uuid, String prefName) {
		EntityXml ent = new EntityXml();
		ent.setEntityId(new EntityIdXml(extId));
	...
	public void importNewTest() {
        ...
        externalSystemVO.setType(ApExternalSystemType.CAM_V2);
        ...
        TransactionTemplate tt = new TransactionTemplate(transactionManager).execute(a -> {
            ...
            Map<String, EntityXml> entXmlMap = IMPORT_ENTITIES.getEntity().stream().collect(Collectors.toMap(
            		ie -> String.valueOf(ie.getEntityId().getValue()), Function.identity()));
