...
static GroovyResult generate(final GroovyPart part) {
    ...
    for (GroovyItem item : part.getItems("IDN_TYPE")) {
        if (item.spec != null && ( item.spec.equals("Kód CZ_RETRO") || item.spec.equals("NUTS/LAU") || item.spec.equals("ISO 3166-1 numeric") || item.spec.equals("ISO 3166-1 alpha-3") || item.spec.equals("ISO 3166-1 alpha-2") || item.spec.equals("číslo archivu") ) ) {
            addIdent = true
            break
        }
    }
