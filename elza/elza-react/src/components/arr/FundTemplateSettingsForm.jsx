        ...
        return <Form className="templates-form" onSubmit={handleSubmit(this.submitReduxForm)}>
            ...
            <Modal.Body className="template-items">
                {templates.map((val, index) => {
                    const header = <div onClick={(e) => {
                        if (edit[index]) {
                            e.preventDefault();
                            e.stopPropagation();
                        }
                    }} className="pull-left template-name">{edit[index] ? <FormInput type="text"
                                                                                     label={false} {...val.name} {...decorateFormField(val.name)} /> : val.name.value}</div>;
