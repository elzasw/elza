...
class AddUserForm extends AbstractReactComponent {
    ...
    render() {
        ...
        return <Form className="add-user-form" onSubmit={handleSubmit(this.submitReduxForm)}>
            <Modal.Body>
                {create && <Row>
                    <Col xs={12}><PartyField disabled={submitting} label={i18n('admin.user.add.party')} {...party}
                                             onCreate={this.handlePartyCreate} detail={false}/></Col>
                </Row>}
                <Row>
                    <Col xs={12}><FormInput disabled={submitting} label={i18n('admin.user.add.username')}
                                            autoComplete="off" type="text" {...username} /></Col>
                </Row>
                <Row className="type-row-group">
                    <Col xs={12}><span className="type">Způsob přihlášení</span></Col>
                </Row>
                <FormCheck disabled={submitting} label={i18n('admin.user.add.password.checkbox')}
                           type="checkbox" {...passwordCheckbox}>{i18n('admin.user.add.password.checkbox')}</FormCheck>
                {passwordCheckbox.value && (create || setPassword) && <Row className="type-row">
                    <Col xs={6}><FormInput disabled={submitting}
                                           label={i18n(create ? 'admin.user.password' : 'admin.user.newPassword')}
                                           autoComplete="off" type="password" {...password} /></Col>
                    <Col xs={6}><FormInput disabled={submitting} label={i18n('admin.user.passwordAgain')}
                                           autoComplete="off" type="password" {...passwordAgain} /></Col>
                </Row>}
                {passwordCheckbox.value && !create && !setPassword && <Row className="type-row">
                    <Col xs={6} className="message">{i18n('admin.user.add.password.message')}</Col>
                    <Col xs={6}><Button disabled={submitting}
                                        onClick={() => this.setState({setPassword: true})}>{i18n('global.action.change')}</Button></Col>
                </Row>}
                <FormCheck disabled={submitting} label={i18n('admin.user.add.shibboleth.checkbox')}
                           type="checkbox" {...shibbolethCheckbox}>{i18n('admin.user.add.shibboleth.checkbox')}</FormCheck>
                {shibbolethCheckbox.value && (create || setShibboleth) && <Row className="type-row">
                    <Col xs={12}><FormInput disabled={submitting} label={i18n('admin.user.add.shibboleth')}
                                            autoComplete="off" type="text" {...shibboleth} /></Col>
                </Row>}
                {shibbolethCheckbox.value && !create && !setShibboleth && <Row className="type-row">
                    <Col xs={6} className="message">{i18n('admin.user.add.shibboleth.message')}</Col>
                    <Col xs={6}><Button disabled={submitting}
                                        onClick={() => this.setState({setShibboleth: true})}>{i18n('global.action.change')}</Button></Col>
                </Row>}
            </Modal.Body>
            <Modal.Footer>
                <Button type="submit"
