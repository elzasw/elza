...
class PasswordForm extends AbstractReactComponent {
    ...
    render() {
        ...
        return <Form onSubmit={handleSubmit(this.submitReduxForm)}>
            <Modal.Body>
                {!admin && <FormInput label={i18n('admin.user.oldPassword')} autoComplete="off"
                                      type="password" {...oldPassword} />}
                <FormInput label={i18n('admin.user.newPassword')} autoComplete="off" type="password" {...password} />
                {!admin && <FormInput label={i18n('admin.user.passwordAgain')} autoComplete="off"
                                      type="password" {...passwordAgain} />}
            </Modal.Body>
            <Modal.Footer>
