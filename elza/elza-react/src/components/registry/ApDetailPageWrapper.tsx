...
//import {APPROVE_AE_DETAIL_AREA, EDIT_AE_DETAIL_AREA} from '../constants';
...
import {
    ApPartFormVO, ApPartNameVO,
    PartType,
} from '../../api/generated/model';
import * as AePartInfo from "../../api/old/ApPartInfo";
import {PartSectionsType} from "../../api/old/ApPartInfo";
...
import AddNodeForm from "../arr/nodeForm/AddNodeForm";
...
import {filterPartFormForSubmit} from "./Detail/partutils";
import {sortItems} from "../../utils/itemutils";
...
const ApDetailPageWrapper: React.FC<Props> = props => {
  ...
  const handleSetPreferred = (part: ApPartNameVO) => {
    if (part.id) {
  ...
  const handleEdit = (part: ApPartVO) => {
    part.id && props.showPartEditModal(props.area, part, props.id, apTypeId, props.codelist, part.partParentId);
    props.invalidateValidationErrors(props.id);
  };
  ...
  const getPartSections = (partType: PartType): ApPartVO[] => {
    if (partSections) {
      let parts: ApPartVO[] = [];
      switch (partType) {
        case PartType.BODY:
      ...
      return parts.filter(value => !value.partParentId);
    } else {
      return [];
    }
  };

  const getRelatedPartSections = (parentPartType: PartType): ApPartVO[] => {
    ...
    const allParts = props.detail.data ? props.detail.data.content as ApPartVO[] : [];
...
const mapDispatchToProps = (
  ...
  showPartEditModal: (area: string, part: ApPartVO, aeId: number, apTypeId: number, codelist: any, parentPartId?: number) => {
              ...
              <PartEditForm
                  ...
                  onSubmit={(formData: ApPartFormVO) => {
                      if (part.id) {
                          ...
                          return WebApi.updatePart(aeId,part.id, filterPartFormForSubmit(formData)).then(() => {
  ...
  deleteParts: async (area: string, aeId: number, parts: ApPartVO[]) => {
    for (let part of parts) {
      if (part.id) {
        await WebApi.deletePart(aeId, part.id);
      }
    }
...
const mapStateToProps = (state: any, props: OwnProps) => {
  ...
  return {
    detail: storeFromArea(state, registry.AREA_REGISTRY_DETAIL),
