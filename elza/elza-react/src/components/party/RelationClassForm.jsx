...
import {Form, FormCheck, FormGroup, Modal} from 'react-bootstrap';
...
class RelationClassForm extends AbstractReactComponent {
    ...
    submitReduxForm = (values, dispatch) =>
        submitForm(RelationClassForm.validate, values, this.props, this.props.onSubmitForm, dispatch);
    ...
    render() {
        const {
            ...
            onClose,
            handleSubmit,
            fields: {from, to, relationEntities, note, source, relationTypeId},
            partyId,
            submitting,
            apTypesMap,
        } = this.props;
                ...
                <Modal.Body className="dialog-3-col relation-form">
                    <div className="flex">
                        <div className="flex-2 col">
                            <FormGroup
                                validationState={relationTypeId.touched && relationTypeId.invalid ? 'error' : null}
                            >
                                <div className="relation-class-select">
                                    {relationTypes.map(i => (
                                        <FormCheck
                                            type={'radio'}
                                            inline
                                            ...
                                            value={i.id}
                                            onBlur={() => {
                                                /* musí být kvůli chybné implementaci v REDUX */
                                            }}
                                            label={i.name}
                                        />
                                    ))}
                                </div>
                                {relationTypeId.touched && relationTypeId.invalid && (
                                    <Form.Control.Feedback>{relationTypeId.error}</Form.Control.Feedback>
                                )}
                            </FormGroup>
                            {relationType && (
                        ...
                        {relationType && relationType.useUnitdate !== USE_UNITDATE_ENUM.NONE && (
