...
class PartyIdentifierForm extends AbstractReactComponent {
    ...
    submitReduxForm = (values, dispatch) =>
        submitForm(PartyIdentifierForm.validate, values, this.props, this.props.onSubmitForm, dispatch);

    render() {
        const {
            onClose,
            handleSubmit,
            fields: {source, note, identifier, from, to},
        } = this.props;
                        ...
                        <div className="flex-2 col">
                            <Row>
                                <Col xs={12}>
                                    ...
                                    <FormInput
                                        type="text"
                                        label={i18n('party.identifier.identifierText')}
                                        {...identifier}
                                    />
                                </Col>
                            </Row>
                                ...
                                <Col xs={12}>
                                    <Row>
                                        <Col xs={6} md={12}>
                                            <DatationField
                                                fields={from}
                                                label={i18n('party.identifier.from')}
                                                labelTextual={i18n('party.identifier.from.textDate')}
                                                labelNote={i18n('party.identifier.from.note')}
                                            />
                                        </Col>
                                        <Col xs={6} md={12}>
                                            <DatationField
                                                fields={to}
                                                label={i18n('party.identifier.to')}
                                                labelTextual={i18n('party.identifier.to.textDate')}
                                                labelNote={i18n('party.identifier.to.note')}
                                            />
                                        </Col>
                                    </Row>
                                </Col>
