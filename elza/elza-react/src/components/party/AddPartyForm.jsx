...
class AddPartyForm extends AbstractReactComponent {
    ...
    validate = values => {
        ...
        if (values.prefferedName.complements && values.prefferedName.complements.length > 0) {
            ...
            errors.prefferedName.complements = values.prefferedName.complements.map(
                AddPartyForm.requireFields('complementTypeId', 'complement'),
            );
            if (errors.prefferedName.complements.filter(i => Object.keys(i).length !== 0).length === 0) {
                delete errors.prefferedName.complements;
            }
        }
        errors.prefferedName !== undefined &&
            Object.keys(errors.prefferedName).length === 0 &&
    ...
    render() {
        ...
        const {
            fields: {
                accessPoint: {typeId, scopeId},
                prefferedName: {nameFormType, degreeBefore, degreeAfter, mainPart, otherPart, partyNameComplements},
            },
        ...
        const complementsList =
            complementsTypes &&
            complementsTypes.map(i => (
                <option value={i.complementTypeId} key={'index' + i.complementTypeId}>
                    {i.name}
                </option>
            ));
                    ...
                    <div className="flex">
                        <div className="flex-2 col">
                            <Row>
                                ...
                                {partyType.code === PARTY_TYPE_CODES.PERSON && (
                                    <Col xs={12}>
                                        <Row>
                                            <Col xs={12} md={6}>
                                                <FormInput
                                                    type="text"
                                                    label={i18n('party.name.degreeBefore')}
                                                    {...degreeBefore}
                                                />
                                            </Col>
                                            <Col xs={12} md={6}>
                                                <FormInput
                                                    type="text"
                                                    label={i18n('party.name.degreeAfter')}
                                                    {...degreeAfter}
                                                />
                                            </Col>
                                        </Row>
                                ...
                                <Col xs={12}>
                                    ...
                                    {partyNameComplements.map((complement, index) => (
                                        <div className="complement" key={'complement' + index}>
                ...
                <Modal.Footer>
                    {showSubmitTypes && (
                        <Button
                            type="submit"
                            onClick={() => {
                                this.submitType = 'storeAndViewDetail';
                            }}
                            disabled={submitting}
                        >
                    ...
                    <Button variant="link" onClick={this.handleClose} disabled={submitting}>
