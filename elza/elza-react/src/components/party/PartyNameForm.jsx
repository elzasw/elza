...
class PartyNameForm extends AbstractReactComponent {
    ...
    static validate = function(values) {
        ...
        errors.partyNameComplements = values.partyNameComplements.map(
            PartyNameForm.requireFields('complementTypeId', 'complement'),
        );
    ...
    dataRefresh = (props = this.props) => {
        const {
            refTables: {partyNameFormTypes, calendarTypes},
            partyTypeId,
        } = props;
        ...
        if (!this.state.initialized) {
            this.setState({initialized: true, complementsTypes: partyType.complementTypes}, () => {
                ...
                for (let f in defaultInitData) {
                    if (!initData[f]) {
                        // Přepsání prázdných položek výchozí hodnotou
    ...
    submitReduxForm = (values, dispatch) =>
        submitForm(PartyNameForm.validate, values, this.props, this.props.onSubmitForm, dispatch);
                                ...
                                <Col xs={12}>
                                    <Row>
                                        <Col xs={6} md={12}>
                                            <DatationField
                                                fields={validFrom}
                                                label={i18n('party.name.validFrom')}
