...
class PartyField extends AbstractReactComponent {
    ...
    handleSearchChange = debounce(text => {
        ...
        WebApi.findParty(
            ...
            this.props.partyTypeId,
            null,
            0,
            AUTOCOMPLETE_PARTY_LIST_SIZE,
        ).then(json => {
            this.setState({
                partyList: json.rows,
                count: json.count,
    ...
    renderParty = props => {
        const {
            refTables: {
                partyTypes: {relationTypesForClass},
            },
        } = this.props;
                ...
                {buttons && (
                    <div className="create-party">
                        <DropdownButton
                            title={
                                <div>
                                    ...
                                    <span className="create-party-label">{i18n('party.addParty')}</span>
                                </div>
                            }
                            id="party-field"
                        >
                            {refTables.partyTypes.items.map(type => (
                                <Dropdown.Item
                                    key={'party' + type.id}
                        ...
                        <Button onClick={this.handleImport} type="button">
                            <Icon glyph="fa-plus" /> {i18n('ribbon.action.party.importExt')}
                        </Button>
                    </div>
                )}
                {count > AUTOCOMPLETE_PARTY_LIST_SIZE && (
                    <div className="items-count">
                        {i18n('partyField.visibleCount', this.state.partyList.length, this.state.count)}
                    </div>
                )}
                {count === 0 && <div className="items-count">{i18n('partyField.noItemsFound')}</div>}
            </div>
        ) : null;
    };
    ...
    handleImport = () => {
        ...
        this.props.dispatch(
            modalDialogShow(
                ...
                <ExtImportForm isParty={true} versionId={versionId === null ? -1 : versionId} />,
                'dialog-lg',
            ),
        );
    };
