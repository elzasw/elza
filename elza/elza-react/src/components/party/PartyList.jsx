            ...
            <div className="party-list">
                <div className="filter">
                    <Autocomplete
                        inputProps={{
                            placeholder:
                                this.getScopeById(partyList.filter.scopeId, scopes) || i18n('party.recordScope'),
                        }}
                        items={this.getScopesWithAll(scopes)}
                        onChange={this.handleFilterPartyScope}
                        value={
                            scopes && partyList.filter.scopeId
                                ? this.getScopeById(partyList.filter.scopeId, scopes)
                                : 'registry.all'
                        }
                    />
                    <Autocomplete
                        inputProps={{
                            placeholder: partyList.filter.state
                                ? StateApproval.getCaption(partyList.filter.state)
                                : i18n('party.apState'),
                        }}
                ...
                {list}
                {isFetched && partyList.rows.length > maxSize && (
                    <span className="items-count">
                        {i18n('party.list.itemsVisibleCountFrom', partyList.filteredRows.length, partyList.count)}
                    </span>
                )}
                {partyList.count > maxSize && (
                    <ListPager
...
export default connect(state => {
    const {
        ...
        userDetail,
        focus,
        refTables: {partyTypes, scopesData},
    } = state;
    return {
        focus,
        partyList,
