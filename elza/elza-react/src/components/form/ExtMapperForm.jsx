...
class ExtMapperForm extends AbstractReactComponent {
    ...
    static validate = data => {
        ...
        error.mappings = data.mappings.map(i => {
            ...
            const entities = i.entities
                ? i.entities.map(e =>
                      e.importEntity && !e.relationRoleTypeId
                          ? {relationRoleTypeId: i18n('global.validation.required')}
                          : null,
                  )
                : [];
    ...
    render() {
        ...
        if (
            ...
            mappings.filter(
                i =>
                    i.relationTypeId.value === null ||
                    i.entities.filter(e => e.relationRoleTypeId.value === null).length > 0,
            ).length > 0
        ) {
            return (
                <div>
        ...
        return (
            <Form name="extMapperForm" onSubmit={handleSubmit}>
                <Modal.Body>
                    <label>{i18n('extMapperForm.relationMapping')}</label>
                    <div style={{border: '1px solid #ddd', maxHeight: '500px', overflowY: 'auto'}}>
                        <Table>
                            {mappings.map(i => {
                                const relationType = i.relationTypeId.value
                                    ? objectById(partyType.relationTypes, i.relationTypeId.value)
                                    : null;
                                const relationRoleTypes =
                                    relationType && relationType.relationRoleTypes
                                        ? relationType.relationRoleTypes
                                        : [];
                                return (
                                    <tbody>
                                        <tr>
                                                ...
                                                <FormInput
                                                    ...
                                                    onChange={ev => {
                                                        const save = window.confirm(
                                                            i18n('extMapperForm.saveAsDefaultMapping'),
                                                        );
                                            ...
                                            i.entities.map(e => (
                                                <tr className="import-relation">
                                                    ...
                                                    <td>
                                                        <FormInput
                                                            as="select"
                                                            disabled={!hasPermission || !i.relationTypeId.value}
                                                            {...e.relationRoleTypeId}
                                                            onChange={ev => {
                                                                const save = window.confirm(
                                                                    i18n('extMapperForm.saveAsDefaultMapping'),
                                                                );
                                                        ...
                                                        >
                                                            <option key="null" />
                                                            {i.relationTypeId.value &&
                                                                relationRoleTypes.map(r => (
                                                                    <option value={r.id} key={r.id}>
                                                                        {r.name}
                                                                    </option>
                                                                ))}
