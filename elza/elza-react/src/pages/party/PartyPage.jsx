...
class PartyPage extends AbstractReactComponent {
     ...
     */
    handleDeleteParty = () => {
        window.confirm(i18n('party.delete.confirm')) &&
            this.props.dispatch(partyDelete(this.props.partyDetail.data.id));
    };

    /* MCV-45365
    handleSetValidParty = () => {
        window.confirm(i18n('party.setValid.confirm')) && this.props.dispatch(setValidParty(this.props.partyDetail.data.id));
    };
    */
    ...
    buildRibbon = () => {
        ...
        if (userDetail.hasOne(perms.AP_SCOPE_WR_ALL)) {
                ...
                <ControllableDropdownButton
                    ...
                    title={
                        <span className="dropContent">
                            ...
                            <div>
                                <span className="btnText">{i18n('party.addParty')}</span>
                            </div>
                        </span>
                    }
                >
                    {partyTypes.items.map(type => (
                        <Dropdown.Item
                            key={type.id}
                            eventKey={type.id}
                            onClick={this.handleAddParty.bind(this, type.id)}
                        >
                            {type.name}
                        </Dropdown.Item>
                    ))}
            ...
            altActions.push(
                <Button key="import-party" onClick={this.handleImport}>
                    <Icon glyph="fa-download fa-fw" />
                    <div>
                        <span className="btnText">{i18n('ribbon.action.party.import')}</span>
                    </div>
                </Button>,
            );
            if (extSystems && extSystems.length > 0) {
                altActions.push(
                    <Button key="import-ext-party" onClick={this.handleExtImport}>
                        <Icon glyph="fa-download fa-fw" />
                        <div>
                            <span className="btnText">{i18n('ribbon.action.party.importExt')}</span>
                        </div>
                    </Button>,
                );
            }
        ...
        if (isSelected && partyDetail.fetched && !partyDetail.isFetching) {
            if (
                ...
                itemActions.push(
                    <Button
                        disabled={partyDetail.data.accessPoint.invalid}
                        key="delete-party"
                        onClick={this.handleDeleteParty}
                    >
                        <Icon glyph="fa-trash" />
                        <div>
                            <span className="btnText">{i18n('party.delete.button')}</span>
                        </div>
                    </Button>,
                );
                ...
                this.props.onShowUsage &&
                    ...
                    itemActions.push(
                        <Button key="partyShow" onClick={() => this.props.onShowUsage(partyDetail)}>
                            ...
                            <div>
                                <span className="btnText">{i18n('party.usage.button')}</span>
                            </div>
                        </Button>,
                    );
...
function mapStateToProps(state) {
    const {
        app: {partyList, partyDetail, apExtSystemList},
        splitter,
        refTables,
        userDetail,
        focus,
