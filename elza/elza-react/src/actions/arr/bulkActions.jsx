...
export function bulkActionsLoadData(versionId, silent = false) {
    return dispatch => {
        ...
        barrier(WebApi.getBulkActions(versionId), WebApi.getBulkActionsState(versionId))
            .then(data => {
                return {
                    actions: data[0].data,
                    states: data[1].data,
...
export function bulkActionsValidateVersion(versionId, silent = false) {
    return dispatch => {
        ...
        barrier(WebApi.bulkActionValidate(versionId), WebApi.getBulkActionsState(versionId))
            .then(data => {
                return {
                    actions: data[0].data,
                    states: data[1].data,
            ...
            .then(json => {
                dispatch(
                    bulkActionsVersionValidateDataReceived(
                        versionId,
                        {actions: json.actions, states: json.states},
                        true,
                    ),
                );
            });
