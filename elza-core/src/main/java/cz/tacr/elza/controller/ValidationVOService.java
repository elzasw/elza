...
public class ValidationVOService {
    ...
    public void checkParty(final ParPartyVO partyVO) {
        ...
        if (partyVO.getPartyType() != null && partyVO.getPartyType().getId() != null) {
            partyType = partyTypeRepository.getOne(partyVO.getPartyType().getId());
        } else {
    ...
    public void checkPartyUpdate(final ParPartyVO partyVO) {
        ParPartyType partyType;
        if (partyVO.getPartyType().getId() != null) {
            partyType = partyTypeRepository.getOne(partyVO.getPartyType().getId());
        } else {
            throw new IllegalArgumentException("Nenalezen typ osoby s id: " + partyVO.getId());
        }
        ...
        ParParty parParty = partyRepository.getOne(partyVO.getId());
    ...
    public void checkRelation(final ParRelationVO relation) {
        ...
        Assert.notNull(relation.getRelationType(), "Není nastaven typ vztahu.");
        Assert.notNull(relation.getRelationType().getId(), "Není nastaven typ vztahu.");
        ...
        Integer relationTypeId = relation.getRelationType().getId();
