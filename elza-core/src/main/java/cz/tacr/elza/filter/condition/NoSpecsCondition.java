...
public class NoSpecsCondition implements HibernateDescItemCondition {
    ...
    public Query createHibernateQuery(final EntityManager entityManager, final Integer fundId, final Integer descItemTypeId,
        ...
        if (lockChangeId == null) {
            String sql = "select distinct n.node_id  from arr_node n join arr_level l on n.node_id = l.node_id left join arr_desc_item di on n.node_id = di.node_id left join arr_item it on di.item_id = it.item_id left join rul_item_spec s on it.item_spec_id = s.item_spec_id "
                    + "where n.fund_id = :fundId and l.delete_change_id is null and n.node_id not in (select di2.node_id from arr_desc_item di2 join arr_item it2 on di2.item_id = it2.item_id where it2.item_type_id = :descItemTypeId and it2.item_spec_id is not null)";
