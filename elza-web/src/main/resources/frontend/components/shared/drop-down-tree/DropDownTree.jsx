...
var DropDownTree = class DropDownTree extends React.Component {
    ...
    renderNode(item, depth){
        ...
        if(item.childrens && item.childrens.length>0 && isOpened){
            for(var i = 0; i<item.childrens.length; i++){
                subList[subList.length] = this.renderNode(item.childrens[i], depth+1);
            };
        }
        return  <li
                    key={item.id}
                ...
                >
                    <span className={"switcher " + (item.childrens && item.childrens.length>0 ? "enabled" : "") } onClick={this.handleToggleNode.bind(this,item)}>{item.childrens && item.childrens.length>0 ? (isOpened ? "−" : "+") : ''}</span>
                    <span 
    ...
    getOpenedDefault(item){
        ...
        if(item.childrens && item.childrens.length>0){
            for(var i = 0; i<item.childrens.length; i++){
                opened = opened.concat(this.getOpenedDefault(item.childrens[i]));
            };
        }
    ...
    getItemLabel(selectedItemID, item = false){
        ...
        if(!item){
            ...
            if(item.childrens && item.childrens.length>0){
                for(var i = 0; i<item.childrens.length; i++){
                    var childLabel = this.getItemLabel(selectedItemID, item.childrens[i])
                    if(childLabel != ''){
                        label = childLabel;
                    };
                };
